...
def boxes_overlap(box_1, box_2):
    return not any(max1 <= min2 or max2 <= min1 for (min1, max1), (min2, max2) in zip(box_1, box_2))
...
def get_overlap(box_1, box_2):
    if not boxes_overlap(box_1, box_2):
        return None
    return [(max(min1, min2), min(max1, max2)) for (min1, max1), (min2, max2) in zip(box_1, box_2)]
...
def split_up_box_from_others(box, other_boxes):
    ...
    for turn_on_or_off, other_box in other_boxes:
        ...
        split_boxes = [
            split for old_split in split_boxes for split in chop_up_box(old_split, overlap)
        ]
...
def is_inside_other(small_box, other_box):
    return all(
        omin <= smin < smax <= omax for (smin, smax), (omin, omax) in zip(small_box, other_box)
    )
...
def parse_line(line):
    ...
    (xmin, xmax), (ymin, ymax), (zmin, zmax) = [
        [int(e) for e in part.split("=")[1].split("..")] for part in box_description.split(",")
    ]
